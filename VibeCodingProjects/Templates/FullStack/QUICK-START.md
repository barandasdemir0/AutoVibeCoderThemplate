...
> **DİKKAT YAPAY ZEKA:** EĞER BU PROJEDE ÖLÇEKLENEBİLİR (SCALABLE) BİR `FULLSTACK MONOREPO VEYA TYPE-SAFE` ALTYAPI ÇALIŞTIRIYORSAN, AŞAĞIDAKİ (ZORUNLU İZOLASYON VE TIP) KURALLARINA %100 UYMAK ZORUNDASIN. BİRBİRİNİ TANIMAYAN REST API'LER VEYA SPAGETTİ REPOLAR ÜRETİRSEN İŞLEM İPTAL OLUR. BİZ MÜKEMMEL FULL STACK ÜRETİMİ İSTİYORUZ!.
...
2. ❌ **REST API (TİP'SİZ) ÇAĞRILARI - HARFİYEN YASAKTIR:** 
   ...
   *DOĞRUSU:* tRPC Tarzı Bir Köprü Kurulacak. Frontend Arka Plandaki Kodu Sadece Oraya Has Tip Uyumlu Methodla: `api.users.create.useMutation( payload )` Formatında Tam Typeli Uyarılı Şekilde Çağıracaktır!!! Harf Hatası (Kopukluk) Cıkan Uç Yok edileceeek!!

3. ❌ **PACKAGE (ÖZEL KÜTÜPHANE) İÇİNDEN APP ÇEKMEK YASAKTIR (CIRCULAR):** 
   Eğer sen "Shared (Ortak)" klasörünün (packages) İçine gidip, UYGULAMANIN İçideki BİR ÇEVİRİ DOSYASINI (App/i18n) Import edersen SISTEMI BAŞTAN SONA Kilitlersin (Circular İmport Sızıntısı)). Paketler ASLA APP lerden Içe aktarma (Dependency) Alamaz. SADECE Uygulamalar Paketi Ceker!!. The Tek Yönlü Hiyerarşi!!.
...
/my-fullstack-repo (Turborepo Kökenli)
 ...
 └── turbo.json         => İŞLEM SIRASI VE CACHE VITES KUTUSU!
```
...
3. Uygulamayı Kök dizinden TEK Bir Komutla (`npm run dev`) Tüm Parçaları Beraber Şaha Kaldıran (concurrently Veya turbo run dev) Betiklerini Mutlaka Pürüzsüz AYARLA!. Yoksa Müşteri Projeyi Tek Tek Başlatmak Zorunda Kalır!(Sefillik).
...
**TEKNOLOJİ VE TİP GÜVENLİĞİ GELECEĞİN THE TEK UZLASI DIR, UFAK SIZINTILAR MİLYONLARCA DOLAR PATLAMALAR YAPAR! MÜKEMMEL DİZİLİME BAŞLAYABİLİRSİN!**
