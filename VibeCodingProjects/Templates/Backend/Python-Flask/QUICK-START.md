...
1. ❌ **KÖK DİZİNDE GLOBAL `app.py` BAŞLATMAK HARFİYEN YASAKTIR:** 
...
2. ❌ **ROTALAR (ROUTES / VIEWS) İÇİNE BUSINESS LOGIC (İŞ MANTIĞI) YIĞMAK YASAKTIR:** 
   Zeka; `@users_bp.route('/register')` metodunun içine veritabanı ekleme kodları (`db.session.add(new_user)`), şifreleme fonksiyonları (`bcrypt.hash`) veya harici servis e-posta gönderme komutları YIĞAMAZ. Flask View katmanı "aptal" olmalıdır.
...
3. ❌ **DAİRESEL İÇE AKTARMA (CIRCULAR IMPORT) YAPMAK KANUNSUZDUR:** 
   ...
   **BUNUN YERİNE:** Veritabanı (`SQLAlchemy`), Şemalar (`Marshmallow`), Yetkilendirme (`JWTManager`) dahil BÜTÜN eklentiler KENDİ BAŞLARINA, SADECE saf deklarasyon olarak `extensions.py` dosyasında yaratılır (`db = SQLAlchemy()`). Modüller `import db` çağrısını sadece ve sadece `extensions.py` üzerinden yapar. `app` ile bağlantı, Factory metodundaki `db.init_app(app)` ile sonradan dokunur!
...
5. ❌ **DOĞRUDAN JSON ENJEKSİYONU (NO DTO) YASAKTIR:**
   Kullanıcıdan gelen `request.get_json()` parametreleri doğrudan `User(**data)` şekliyle DB modeline basılamaz! Modeller dışarı sergilenirken `__dict__` ile JSON'a dökülemez (Şifreler ve Metadata sızar).
...
/my_flask_backend
 ...
 │    │    │    ├── services.py  # AĞIR İŞÇİLİK (Business Logic ve DB Commitler)
 ...
 └── run.py                 # MÜŞTERİ YÜZÜ: app = create_app(); app.run() burada atar!
```
...
Bir proje istenildiğinde körü körüne kod yazmaya başlama, ŞU SIRAYI TAKİP ET:
...
```python
# src/modules/auth/routes.py YAKLAŞIMI
...
def register():
    try:
        ...
        # Schema Dump (Dışarıya Şifresiz Çıktı)
...
3. **ŞU ANDAN İTİBAREN BU STANDARTLARA MAHKUMSUN. KOLAY GELSİN OTONOM! 🚀**
