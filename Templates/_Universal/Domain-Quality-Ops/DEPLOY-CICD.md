...
## 🛡️ Branch Protection ve Merge Kuralları

Zorunlu koruma modeli:
- `main` branch protected: direct push kapalı
- `develop` branch protected: direct push kapalı
- Merge sadece Pull Request ile
- Required status checks: `lint`, `test`, `build`
- En az 1 review (insan veya tanımlı kontrol kuralı)

AI ajan kuralları:
- AI yalnız `feature/*` veya `fix/*` branch'inde çalışır
- CI kırmızıysa PR merge edilmez
- Staging smoke test geçmeden `main` merge yapılmaz

---

## 🏷️ Semantic Versioning ve Rollback

Release versiyonlama:
- `vMAJOR.MINOR.PATCH` formatı zorunlu
- Her `main` release'inde tag aç
- `CHANGELOG.md` ve release notları güncel olmalı

Rollback politikası:
1. Sorunlu release tespit et
2. Son stabil tag'e dön
3. Hotfix branch aç (`hotfix/<issue-id>-<ad>`)
4. Düzeltmeyi CI üzerinden geçir
5. Yeni patch release (`vX.Y.(Z+1)`) yayınla
